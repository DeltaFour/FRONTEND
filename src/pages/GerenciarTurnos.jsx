...
import { FaClock, FaPlus, FaEdit, FaTrash, FaTimes, FaSave } from "react-icons/fa";
...
const GerenciarTurnos = () => {
  ...
  const fetchShifts = useCallback(async () => {
    try {
      ...
      const formattedShifts = (response.data.data || response.data).map((shift) => ({
        id: shift.id,
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      setError(
        `Erro ao salvar: ${err.response?.data?.message || "Verifique os dados informados."}`
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const handleDelete = async (shiftId, shiftType) => {
    if (!window.confirm(`Tem certeza que deseja EXCLUIR o turno "${shiftType}"?`)) return;
    try {
  ...
  if (loading) return <div className="text-center py-8">Carregando jornadas de trabalho...</div>;
  if (error && !isModalOpen) return <div className="text-red-600 p-4 bg-red-100 rounded">{error}</div>;

  return (
    <div className="bg-white p-6 rounded-lg shadow-xl">
      <div className="flex justify-between items-center mb-6 border-b pb-3">
        <h2 className="text-2xl font-bold text-gray-800 flex items-center">
          <FaClock className="mr-3 text-indigo-600" /> Gerenciar Jornadas de Trabalho
        </h2>
        <button
          onClick={() => openModal()}
        ...
        <div className="overflow-x-auto border rounded-lg">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nome do Turno</th>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tolerância (min)</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {shifts.map((shift) => (
                <tr key={shift.id}>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{shift.workShiftType}</td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{shift.workShiftToleranceMinutes}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <button onClick={() => openModal(shift)} className="text-blue-600 hover:text-blue-900 mr-3 p-1 inline-flex items-center">
                      <FaEdit className="inline mr-1" /> Editar
                    </button>
                    <button onClick={() => handleDelete(shift.id, shift.workShiftType)} className="text-red-600 hover:text-red-900 p-1 inline-flex items-center ml-2">
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-white p-8 rounded-lg shadow-2xl w-full max-w-md">
            <div className="flex justify-between items-center border-b pb-3 mb-4">
              <h3 className="text-xl font-bold text-gray-800">{isEditing ? "Editar Turno" : "Criar Novo Turno"}</h3>
              <button onClick={closeModal} className="text-gray-400 hover:text-gray-600"><FaTimes /></button>
            </div>

            {error && submitting && <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4">{error}</div>}

            <form onSubmit={handleSave} className="space-y-4">
              <div>
                <label htmlFor="workShiftType" className="block text-sm font-medium text-gray-700">Tipo de Turno</label>
                <select
                  name="workShiftType"
                  id="workShiftType"
              ...
              <div>
                <label htmlFor="workShiftToleranceMinutes" className="block text-sm font-medium text-gray-700">Tolerância (minutos)</label>
                <input
                  type="number"
              ...
              <div className="flex justify-end space-x-3 pt-4">
                <button type="button" onClick={closeModal} className="px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 hover:bg-gray-50">Cancelar</button>
                <button type="submit" disabled={submitting} className={`flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 ${submitting ? "opacity-50 cursor-not-allowed" : ""}`}>
                  <FaSave className="mr-2" />{submitting ? "Salvando..." : "Salvar Turno"}
                </button>
              </div>
            </form>
          </div>
