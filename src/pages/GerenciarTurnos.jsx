...
const GerenciarTurnos = () => {
  ...
  const openModal = (shift = null) => {
    if (shift) {
      ...
      setFormData({
        ...
        startTime: `${String(shift.workShiftStartTime.hour).padStart(
          2,
          "0"
        )}:${String(shift.workShiftStartTime.minute).padStart(2, "0")}`,
        endTime: `${String(shift.workShiftEndTime.hour).padStart(
          2,
          "0"
        )}:${String(shift.workShiftEndTime.minute).padStart(2, "0")}`,
  ...
  const handleSave = async (e) => {
    ...
    const endpoint = isEditing ? "/workshift" : "/workshift";
    const method = isEditing ? api.patch : api.post;

    const payload = {
      workShiftType: formData.workShiftType,
      workShiftStartTime: parseTime(formData.startTime),
      workShiftEndTime: parseTime(formData.endTime),
      workShiftToleranceMinutes: Number(formData.workShiftToleranceMinutes),
    };
    ...
    try {
      ...
      console.error("Erro ao salvar turno:", err.response || err);
      setError(
        `Erro ao salvar: ${
  ...
  const handleDelete = async (shiftId, shiftType) => {
    ...
    try {
      await api.delete(`/workshift/${shiftId}`);
      alert(`Turno "${shiftType}" excluído com sucesso!`);
      fetchShifts();
    } catch (err) {
      console.error("Erro ao excluir turno:", err.response || err);
      alert(
        `Erro ao excluir: ${err.response?.data?.message || "Erro de API."}`
      );
    }
  };
              ...
              {shifts.map((shift) => (
                <tr key={shift.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {String(shift.workShiftStartTime.hour).padStart(2, "0")}:
                    {String(shift.workShiftStartTime.minute).padStart(2, "0")}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {String(shift.workShiftEndTime.hour).padStart(2, "0")}:
                    {String(shift.workShiftEndTime.minute).padStart(2, "0")}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-gray-500">
                  Minutos de tolerância para a marcação de ponto.
                </p>
              </div>
